...
class HealthDataRepositoryImpl @Inject constructor(
    private val dao: HealthDataDao,
    private val providerFactory: ProviderFactory,
) : HealthDataRepository {
    override suspend fun sync(parentId: String): AppResult<Unit> = try {
        ...
        val entities = metrics.flatMap { metric ->
            provider.fetchSince(parentId, metric, now - 60 * 60_000).map {
                HealthDataEntity(
                    parentId = it.parentId,
                    metricType = it.metric.name,
                    value = it.value,
                    unit = it.unit,
        ...
        AppResult.Error("Health sync failed", t)
    }

    override fun latestMetrics(parentId: String): Flow<List<HealthData>> =
        dao.observeByParent(parentId).map { list ->
